...
const { CONSTANTS, setBit, pack, neighbors, unpack } = require('./util');
 ...
 */
const assertCount = (expected, chunkStore, chunkid, bitpos) => {
    const unpacked = unpack(chunkid, bitpos);
    const count = chunkStore.countNeighboringMines(chunkid, bitpos);
    console.log('testing:', unpacked, count);
 ...
 * @returns {Uint32Array}
 */
const emptyChunkMines = difficulty => new Uint32Array(CONSTANTS.BITFIELD_SIZE);
...
describe('ChunkStore', () => {
    ...
    describe('iterativeReveal', () => {
        it.each([
            ...
            [CONSTANTS.CHUNK_EDGE_SIZE - 2, CONSTANTS.CHUNK_EDGE_SIZE - 2],
