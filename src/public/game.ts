// rather than include "dom" in the full types and deal with it interfering
// with all the server code, we'll just include it this way in this file

/// <reference lib="dom" />

// all the rest of these are mostly type imports, except `splitKey`, so
// we do have some dependency on the other code being browser-compatible.
// this will increase as i integrate the new chunk storage stuff, but
// i've already written those to be browser-compatible

import type { io as _io, Socket } from 'socket.io-client';
import {
    type LegacyChunk,
...
import type {
    ...
    PeriodicLeaderboardUpdate,
    ServerToClientEvents,
} from '../protocol.js';

// socket.io-client gets loaded from the bundled version included
// in the server, but there doesn't seem to be an export for typeof io

declare const io: typeof _io;

// a couple functions get set on window for some reason, make typescript happy
declare global {
    interface Window {
        toggleLeaderboard: typeof toggleLeaderboard;
        closeAdminPopup: typeof closeAdminPopup;
    }
}

// dom / type helper functions

const expectElementById = <T extends HTMLElement = HTMLElement>(
    id: string,
): T => {
...
const setTextContent = (id: string, text: string | number) =>
    (expectElementById(id).textContent = String(text));
const setStyle = <K extends keyof CSSStyleDeclaration>(
    id: string,
    prop: K,
    val: CSSStyleDeclaration[K],
) => (expectElementById(id).style[prop] = val);
const addClass = (id: string, klass: string) =>
    expectElementById(id).classList.add(klass);
const removeClass = (id: string, klass: string) =>
    expectElementById(id).classList.remove(klass);

// previous game code (mostly unaltered)

const urlParams = new URLSearchParams(window.location.search);
const debugToken = urlParams.get('token');
const isDebugPage =
    window.location.pathname.endsWith('/debug') ||
    window.location.pathname.endsWith('/debug.html'); // I cannot fucking remember why i have a debug endpoint and a debug param but whatever.
const isDebugMode =
    (urlParams.get('debug') === '1' && !!debugToken) ||
    (isDebugPage && !!debugToken); // i probably had a separate debug thing before.
const socket: Socket<ServerToClientEvents, ClientToServerEvents> = isDebugMode
    ? io({ auth: { debugToken } })
    : io();
if (!isDebugMode) {
...
const canvas = expectElementById<HTMLCanvasElement>('canvas');
const ctx = canvas.getContext('2d')!;

const CELL_SIZE = 32;
const CHUNK_SIZE = 16;

let player: SerializedPlayer | null = null;
let players = new Map<string, SerializedPlayer>();
let chunks = new Map<string, LegacyChunk>();
let cameraX = 0;
let cameraY = 0;
let mouseWorldX = 0;
let mouseWorldY = 0;
...
let playerCells = new Set<string>();
let clickableCells = new Set<string>();
let isPanning = false;
let panStartX = 0;
let panStartY = 0;
...
let uncoverRevealTimes = new Map<string, number>();
...
socket.on('gameUpdate', data => {
    ...
    if (
        ...
        if (data.type === 'death' && data.playerCells) {
            ...
            if (isDebugMode && data.playerId === player?.id) {
        ...
        for (const update of data.updates) {
            if (update.type === 'respawn' && update.playerId === player?.id) {
                player.x = update.x;
                player.y = update.y;
                player.alive = true;
                isDead = false;
...
socket.on('recoveryComplete', data => {
    ...
    if (data.playerId === player?.id) {
        const finalScore =
            lastDeathScore !== null ? lastDeathScore : player.score;
        displayDeathBanner('You hit a mine!', deathTime!, finalScore);
    }
});
...
function showAdminPopup(title: string, text: string) {
    setTextContent('adminPopupTitle', title);
    setTextContent('adminPopupText', text);
    setStyle('adminPopup', 'display', 'flex');
}

function closeAdminPopup() {
    setStyle('adminPopup', 'display', 'none');
}

window.closeAdminPopup = closeAdminPopup;

function handleDeath(reason: string) {
    isDead = true;
    deathTime = Date.now();

    displayDeathBanner(reason, deathTime, player?.score ?? 0);
}

function displayDeathBanner(
    reason: string,
    deathTime: number,
    finalScore: number,
) {
    ...
    setTextContent('deathReason', reason);
    setTextContent('finalScore', `Final Score: ${finalScore}`);
    addClass('deathScreen', 'show');
    ...
    const timerInterval = setInterval(() => {
        ...
        setTextContent('respawnTimer', remaining);
...
function updateCellData(cellData: LegacyChunkCell) {
...
function scheduleUncoverReveal(cells: LegacyCoord[]) {
...
function invalidateChunksForCells(cells: LegacyCoord[]) {
    ...
    for (const cell of cells) {
        ...
        if (localX === CHUNK_SIZE - 1)
            chunksToInvalidate.add(`${chunkX + 1},${chunkY}`);
        if (localY === 0) chunksToInvalidate.add(`${chunkX},${chunkY - 1}`);
        if (localY === CHUNK_SIZE - 1)
            chunksToInvalidate.add(`${chunkX},${chunkY + 1}`);
    }
...
function requestChunksForCells(cells: LegacyCoord[]) {
...
function markDirtyChunksForCells(cells: LegacyCoord[]) {
    for (const cell of cells) {
        const chunkX = Math.floor(cell.x / CHUNK_SIZE);
        const chunkY = Math.floor(cell.y / CHUNK_SIZE);
        ...
        if (localX === CHUNK_SIZE - 1)
            dirtyChunks.add(`${chunkX + 1},${chunkY}`);
        if (localY === 0) dirtyChunks.add(`${chunkX},${chunkY - 1}`);
        if (localY === CHUNK_SIZE - 1)
            dirtyChunks.add(`${chunkX},${chunkY + 1}`);
    }
}
...
function getVisibleChunkKeys() {
    ...
    const startChunkX =
        Math.floor(
            (cameraX - canvas.width / (2 * zoom)) / (CHUNK_SIZE * CELL_SIZE),
        ) - buffer;
    const endChunkX =
        Math.floor(
            (cameraX + canvas.width / (2 * zoom)) / (CHUNK_SIZE * CELL_SIZE),
    ...
    const startChunkY =
        Math.floor(
            (cameraY - canvas.height / (2 * zoom)) / (CHUNK_SIZE * CELL_SIZE),
        ) - buffer;
    const endChunkY =
        Math.floor(
            (cameraY + canvas.height / (2 * zoom)) / (CHUNK_SIZE * CELL_SIZE),
...
function requestVisibleChunks() {
    ...
    for (
        let cx = startChunkX;
        cx <= endChunkX && chunkKeys.length < maxChunks;
        cx++
    ) {
        for (
            let cy = startChunkY;
            cy <= endChunkY && chunkKeys.length < maxChunks;
            cy++
        ) {
            const key = `${cx},${cy}`;
...
function updateLeaderboard(leaderboardData: PeriodicLeaderboardUpdate) {
    const content = document.getElementById('leaderboardContent')!;
    if (!leaderboardData || leaderboardData.length === 0) {
        content.innerHTML =
            '<div style="text-align: center; color: rgb(128, 128, 128);">No players</div>';
        return;
    }

    const yourRank = leaderboardData.findIndex(p => p.id === player?.id);
    const lines = [];

    const top10 = leaderboardData.slice(0, 10);
    for (let i = 0; i < top10.length; i++) {
        const isYou = top10[i]!.id === player?.id;
        const className = isYou ? 'leaderboard-entry you' : 'leaderboard-entry';
        const youText = isYou ? ' (you)' : '';
        lines.push(
            `<div class="${className}">${i + 1}. ${top10[i]!.score} points${youText}</div>`,
        );
    }
    ...
    if (yourRank >= 10) {
        ...
        for (let i = contextStart; i < contextEnd; i++) {
            ...
            const youText = isYou ? ' (you)' : '';
            lines.push(
                `<div class="${className}">${i + 1}. ${leaderboardData[i]!.score} points${youText}</div>`,
            );
        }
    }
...
function updatePlayerCells() {
    ...
    for (const chunk of chunks.values()) {
        for (const cell of chunk.cells) {
            if (cell.owner === player?.id && cell.state === 'uncovered') {
...
function isAdjacentToPlayerCell(x: number, y: number) {
...
function render() {
    ...
    for (const chunk of chunks.values()) {
        for (const cell of chunk.cells) {
            ...
            const shouldRenderCovered =
                cell.flag || isAdjacentToPlayerCell(cell.x, cell.y);
        ...
        for (const chunk of chunks.values()) {
            for (const cell of chunk.cells) {
                if (
                    cell.state === 'covered' &&
                    cell.isMine &&
                    isAdjacentToPlayerCell(cell.x, cell.y)
                ) {
                    const screenX = cell.x * scaledCellSize + offsetX;
                    const screenY = cell.y * scaledCellSize + offsetY;
                    ...
                    ctx.lineTo(
                        screenX + scaledCellSize - inset,
                        screenY + scaledCellSize - inset,
                    );
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo(
...
function renderPlayerBorders(
    visibleCells: Map<string, LegacyChunkCell>,
    scaledCellSize: number,
    offsetX: number,
    offsetY: number,
) {
    const thickness = Math.min(
        Math.max(3, Math.round(5 * zoom)),
        Math.floor(scaledCellSize / 2),
    );
...
function renderUncoveredCell(
    cell: LegacyChunkCell,
    screenX: number,
    screenY: number,
    scaledCellSize: number,
    ...
    for (const [deadPlayerId, deadData] of deadPlayerCells.entries()) {
        if (deadData.cells.has(cellKey)) {
            ...
            if (
                deadData.mineCell &&
                cell.x === deadData.mineCell.x &&
                cell.y === deadData.mineCell.y
            ) {
                isMineCell = true;
            }
            break;
...
function renderCoveredCell(
    ...
    if (shouldDraw) {
        ...
        if (!simplifiedRendering) {
            ...
            ctx.lineTo(
                screenX + scaledCellSize - halfLine,
                screenY + scaledCellSize - halfLine,
            );
    ...
    if (cell.flag) {
        if (!simplifiedRendering) {
            ctx.fillStyle = 'rgb(0, 0, 0)';
            ctx.fillRect(
                screenX + scaledCellSize / 2 - 0.5 * zoom,
                screenY + 8 * zoom,
                1 * zoom,
                12 * zoom,
            );
            ...
            ctx.beginPath();
            ctx.moveTo(
                screenX + scaledCellSize / 2 + 1 * zoom,
                screenY + 8 * zoom,
            );
            ctx.lineTo(
                screenX + scaledCellSize / 2 + 1 * zoom,
            ...
            ctx.lineTo(
                screenX + scaledCellSize / 2 + 8 * zoom,
                screenY + 11 * zoom,
            );
            ctx.closePath();
            ctx.fill();
...
setInterval(() => {
    const cameraMoved =
        Math.abs(cameraX - lastCameraX) > CELL_SIZE * 2 ||
...
let lastTapTime = 0;
let lastTapCell: string | null = null;
let lastClickTime = 0;
let lastClickCell: string | null = null;
let lastDeathScore: number | null = null;
