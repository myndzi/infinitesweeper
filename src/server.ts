...
import type { LegacyPlayerData } from './game/legacy.js';
import type {
...
const AI_PLAYER_COUNT = Math.max(
    0,
    parseInt(process.env.AI_PLAYER_COUNT || '0', 10) || 0,
);
...
function isValidAdminToken(token: any) {
...
io.on('connection', socket => {
    ...
    const isAdmin =
        socket.handshake.auth && socket.handshake.auth.role === 'admin';
    const debugToken =
        socket.handshake.auth && socket.handshake.auth.debugToken;
    ...
    function emitPlayerInit(playerData: LegacyPlayerData) {
        socket.emit('init', {
            playerId: socket.id,
    ...
    function initializePlayerAt(x: number, y: number) {
        if (playerInitialized) return;
        playerInitialized = true;
    ...
    socket.on('requestChunks', data => {
        ...
        const chunks = game.getChunks(
            data.chunkKeys,
            includeMines ? { includeMines: true } : null,
        );
    ...
    socket.on('debugSpawn', data => {
        ...
        if (!playerInitialized) {
            ...
            if (player && !player.alive) {
                ...
                if (
                    !game.grid.hasOtherPlayerNearby(
                        data.x,
                        data.y,
                        socket.id,
                        game.safeRadius,
                ...
                const uncoverResult = game.grid.uncoverCell(
                    data.x,
                    data.y,
                    socket.id,
                );
    ...
    socket.on('adminBroadcast', data => {
        if (!data || !data.token) {
            socket.emit('adminBroadcastResult', {
                success: false,
                error: 'Unauthorized',
            });
            return;
    ...
    socket.on('adminSetAIPlayers', data => {
        if (!data || !data.token) {
            socket.emit('adminSetAIPlayersResult', {
                success: false,
                error: 'Unauthorized',
            });
            return;
        ...
        const target = Math.max(
            0,
            parseInt(String(data.count) || '0', 10) || 0,
        );
